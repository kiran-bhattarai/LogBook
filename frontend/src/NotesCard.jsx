...
function NotesCard({ note, onRealClick }) {
    ...
    return (
        ...
        < div className="flex justify-start flex-wrap relative group" >
            ...
            { notDeleted &&
                <div onClick={() => onRealClick(note)} className="mb-2 flex flex-col min-w-56 max-w-160 w-full bg-neutral-800 justify-self-center p-1 border border-neutral-400 rounded relative hover:shadow-[0_0px_10px_rgba(255,255,255,0.35)] transition duration-300 justify-between">
                    ...
                    <input type="text" id="" placeholder={"(No Title)"} className={`justify-self-start border border-t-0 border-x-0 w-[90%] px-2 text-2xl py-1 transition duration-200 text-white outline-none text-ellipsis border-b-neutral-600`} value={note.title} readOnly />

                    <textarea ref={textareaRef} maxLength={3000} placeholder={"(No Body)"} className={`resize-none w-full text-lg outline-none scrollbar text-white px-2 mt-0 py-1 max-h-[240px] overflow-y-auto`} value={note.body} readOnly></textarea>
