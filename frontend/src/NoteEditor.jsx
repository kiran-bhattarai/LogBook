    ...
    return (
        <div>
            <div className="h-[80%] w-[80%] flex flex-col rounded-xl bg-neutral-800 absolute justify-self-center top-1/2 -translate-y-1/2 p-4 shadow-[0px_0px_16px_black]">
                <input type="text" name="" value={title} id="" onChange={(e)=> setTitle(e.target.value)} placeholder={"(No Title)"} className={`border border-t-0 border-x-0 w-full px-2 text-2xl py-1 transition duration-200 text-white outline-none border-b-neutral-600 font-medium`} />
                <textarea name="" value={body} maxLength={3000} onChange={(e)=> setBody(e.target.value)} id="" placeholder={"(No Body)"} className={` grow resize-none w-full text-lg outline-none scrollbar text-white px-2 mt-1 py-1 overflow-y-auto`} ></textarea>
