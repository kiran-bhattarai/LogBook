...
function SignupPage({ setLoginWindow, setItselfOff }) {
    ...
    return (
        <div ref={containerRef}>
            <div className=" p-2 pb-3 bg-white relative rounded-xl flex flex-col items-center w-[clamp(350px,50vmin,400px)] ">

                <h1 className="text-3xl font-inter text-[#444444] font-medium mb-[clamp(48px,7vh,68px)] ">Sign up</h1>
                <span className={`absolute top-13 text-[clamp(14px,2vmin,16px)] text-red-600 font-medium text-center leading-tight px-6`}>{messageInfo}</span>
                ...
                <form onSubmit={handleSubmit} noValidate className="w-full px-[clamp(8px,1vw,16px)] items-center flex flex-col">
                    ...
                    <div className="w-full overflow-clip flex items-center relative justify-center max-w-114">
                        ...
                        <input type="text" placeholder="Name" className={`bg-[#dfdfdf] py-[clamp(2px,(0.4vh),4px)] m-1 w-[99%] px-10 rounded-4xl  outline-2 outline-[#dddddd] text-[clamp(19px,2.1vmin,24px)] text-ellipsis  ${getOutlineColor(nameTouched, isValidName())} pr-15 mx-1 text-ellipsis transition duration-300`} onChange={(e) => handleNameChange(e)} maxLength={20} minLength={3} onBlur={() => setNameTouched(true)} />

                        <span className={`absolute h-7 bottom-1/2 translate-y-1/2 right-3.5 text-[clamp(15px,1.8vmin,19px)] font-medium ${nameLength === 0 ? "text-neutral-500" : (nameLength < 3 || nameLength > 20) ? "text-red-600" : "text-green-700"} `}>{nameLength}/20</span>
                    </div>
                    <div className="w-full overflow-clip flex items-center justify-center relative max-w-114">
                        ...
                        <input placeholder="Email" className={`bg-[#dfdfdf] py-[clamp(2px,(0.4vh),4px)] m-1 w-[99%] px-10 rounded-4xl  text-[clamp(18px,2.1vmin,22px)] outline-[#dddddd] outline-2 text-ellipsis ${getOutlineColor(emailTouched, isValidEmail())} mx-1 text-ellipsis transition duration-300`} onChange={(e) => handleEmailChange(e)} onBlur={() => setEmailTouched(true)} />
                    </div>

                    <div className="w-full overflow-clip flex items-center relative justify-center max-w-114">
                        <img src="../src/assets/pass.svg" alt="" className="absolute h-6 left-4" />

                        <input type={passwordType} placeholder="Password" className={`bg-[#dfdfdf] py-[clamp(2px,(0.4vh),4px)] m-1 w-[99%] px-10 rounded-4xl  outline-2 outline-[#dddddd] text-[clamp(19px,2.1vmin,24px)] text-ellipsis ${getOutlineColor(passwordTouched, isValidPassword())} mx-1 text-ellipsis transition duration-300`} onChange={(e) => handlePasswordChange(e)} onBlur={() => { setPasswordRetypeTouched(true); setPasswordTouched(true) }} />
                    ...
                    <div className="w-full overflow-clip flex items-center relative justify-center max-w-114">
                        ...
                        <input type={passwordRetypeType} placeholder="Retype Password" className={`bg-[#dfdfdf] py-[clamp(2px,(0.4vh),4px)] m-1 w-[99%] px-10 rounded-4xl  outline-2 outline-[#dddddd] text-[clamp(19px,2.1vmin,24px)] text-ellipsis ${getOutlineColor(passwordRetypeTouched, isValidPasswordRetype())} mx-1 text-ellipsis transition duration-300`} onChange={(e) => handlePasswordRetypeChange(e)} onBlur={() => { setPasswordRetypeTouched(true); setPasswordTouched(true) }} />
                    ...
                    <span className={`text-[clamp(13px,1.8vmin,16px)] text-neutral-500 text-center leading-tight m-[clamp(8px,1.2vmin,16px)]`}>Password should be atleast 8 characters, including one uppercase letter, one lowercase letter, one number, and one special symbol.</span>
                    ...
                    text-[clamp(20px,2.8vmin,24px)] pt-0.5 pb-1.5 font-semibold rounded-4xl cursor-pointer hover:scale-105 self-auto w-[40%] text-nowrap">Sign up</button>
                </form>
