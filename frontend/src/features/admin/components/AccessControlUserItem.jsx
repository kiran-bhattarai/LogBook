                ...
                <div className="border-b border-neutral-500 flex flex-col rounded bg-white/5 dark:bg-[#202023]/30 pb-1 px-3 pr-2 sm:pr-3 items-center w-full text-black dark:text-white">
                    ...
                    <Link to={`/profile?id=${user._id}`} className="flex justify-between w-full">
                        ...
                        <div className="sm:flex-row flex justify-between items-start sm:items-center flex-col w-full">
                            ...
                            <div className="flex flex-col text-[14.5px] mt-4 sm:mt-1 text-black dark:text-white">
                                <span><span className="text-neutral-800 dark:text-neutral-400">Total notes: </span>{user.notesCount}</span>
                                <span><span className="text-neutral-800 dark:text-neutral-400">Created on: </span>{new Date(user.createdAt).toISOString().split("T")[0] + "," + new Date(user.createdAt).toLocaleString().split(",")[1]}</span>
                            </div>
                        </div>

                        <div className="mt-2 md:ml-5 ml-2 h-min">
                            <span className={`text-white text-lg px-2 pb-1 pt-0.5 rounded-lg ${role === "admin" ? "bg-red-700 dark:bg-red-900" : "bg-green-700 dark:bg-green-900"} font-medium`}>
