...
function NotesCard({ note, onRealClick, deleteNoteLogicMain }) {
    ...
    return (
        <motion.div animate={visible ? { y: 0, opacity: 1 } : { y: -300, opacity: 0 }} transition={{ type: "spring", stiffness: 300, damping: 25, duration: 0.8 }} className="flex justify-center flex-wrap relative group bg-white/2  backdrop-blur-sm" >
            <div onClick={() => onRealClick(note)} className="font-inter flex flex-col min-w-56 max-w-140 w-full dark:bg-white/4 justify-self-center p-1 border bg-neutral-100/30 hover:border-black border-neutral-700 dark:shadow-neutral-700 dark:hover:border-neutral-100 hover:scale-101 dark:border-neutral-500 rounded-2xl relative hover:shadow-[0_0px_10px_rgba(255,255,255,0.35)] transition duration-300 justify-between">
                ...
                <input type="text" id="" placeholder={"(No Title)"} className={`text-black dark:text-white justify-self-start border border-t-0 border-x-0 ${deleteNoteLogicMain && "w-[90%] duration-200"} px-2 text-2xl py-1 transition duration-200 outline-none text-ellipsis border-b-neutral-600 `} value={note.title} readOnly />

                <textarea ref={textareaRef} maxLength={3000} placeholder={"(No Body)"} className={`text-black dark:text-white resize-none font-light w-full text-lg outline-none scrollbar px-2 mt-0 py-1 max-h-60 overflow-y-auto`} value={note.body} readOnly></textarea>
                ...
                <div className={`border border-x-0 border-b-0 border-t-neutral-600 flex w-full py-0.5 items-center justify-between`}>
                    ...
                    <span className={`text-[14px] px-2 text-black dark:text-neutral-300`}>{(note.body?.length || 0) + (note.title?.length || 0)}/3000</span>
