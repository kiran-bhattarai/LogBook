...
function SignupPage({ setLoginWindow, setItselfOff }) {
    ...
    const [passwordRetypeIcon, setPasswordRetypeIcon] = useState(null);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await signupRequest({ name: nameValue, email: emailValue, password: passwordValue, passwordRetype: passwordRetypeValue })
            login(emailValue, passwordValue)
            setItselfOff()
            navigate("/verify-email")
        } catch(err) {
            setMessageInfo(err.response?.data?.message || "Something went wrong")
        }
            ...
            <div className=" p-2 pb-3 bg-white dark:bg-[#202023] dark:text-white relative rounded-xl flex flex-col items-center w-[350px] font-inter">
                ...
                <form onSubmit={handleSubmit} noValidate className="w-full px-[8px] items-center flex flex-col">
                    ...
                    <div className="w-full overflow-clip flex items-center relative justify-center max-w-114">
                        <img src={UserIcon} alt="" className="absolute h-6 left-4" />

                        <input autoComplete="username" type="text" placeholder="Name" className={`bg-[#dfdfdf] dark:bg-[#101010] dark:outline-[#101010] py-[4px] m-1 w-[99%] px-10 rounded-4xl  outline outline-[#dddddd] text-[19px] text-ellipsis  ${getOutlineColor(nameTouched, isValidName())} pr-15 mx-1 text-ellipsis transition duration-300`} onChange={(e) => handleNameChange(e)} maxLength={20} minLength={3} onBlur={() => setNameTouched(true)} />
                    ...
                    <div className="w-full overflow-clip flex items-center justify-center relative max-w-114">
                        ...
                        <input autoComplete="email" placeholder="Email" className={`bg-[#dfdfdf] dark:bg-[#101010] dark:outline-[#101010] py-[4px] m-1 w-[99%] px-10 rounded-4xl  text-[18px] outline-[#dddddd] outline text-ellipsis ${getOutlineColor(emailTouched, isValidEmail())} mx-1 text-ellipsis transition duration-300`} onChange={(e) => handleEmailChange(e)} onBlur={() => setEmailTouched(true)} />
                    </div>
                    <div className="w-full overflow-clip flex items-center relative justify-center max-w-114">
                        <img src={PasswordIcon} alt="" className="absolute h-6 left-4" />

                        <input autoComplete="new-password" type={passwordType} placeholder="Password" className={`bg-[#dfdfdf] dark:bg-[#101010] dark:outline-[#101010] py-[4px] m-1 w-[99%] px-10 rounded-4xl  outline outline-[#dddddd] text-[19px] text-ellipsis ${getOutlineColor(passwordTouched, isValidPassword())} mx-1 text-ellipsis transition duration-300`} onChange={(e) => handlePasswordChange(e)} onBlur={() => { setPasswordRetypeTouched(true); setPasswordTouched(true) }} />
